...
public final class MasterKeys {
    ...
    /**
     * The scheme to base our master key on.
     */
    ...
    /**
     * @deprecated Use {@link KeyScheme#AES256_GCM_SPEC} instead.
     */
    @Deprecated
    @NonNull
     ...
     * encryption used is clear to developers.
     *
     * @param keyScheme The key encryption scheme
     * @return The key alias for the master key
     */
    @RequiresApi(Build.VERSION_CODES.M)
    @NonNull
    public static String getOrCreate(@NonNull KeyScheme keyScheme)
            throws GeneralSecurityException, IOException {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (keyScheme == KeyScheme.AES256_GCM_SPEC) {
                return getOrCreate(createAES256GCMKeyGenParameterSpec(MASTER_KEY_ALIAS));
            } else {
                throw new IllegalArgumentException("Bad key scheme: " + keyScheme.name());
            }
        }
